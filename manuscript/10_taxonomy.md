...
DigiPPPiP is organized by two primary axes: temporal structure and spatial configuration. The configured project has {{RESULT_NUM_TEMPORAL_MODES}} temporal modes, {{RESULT_NUM_SPATIAL_CONFIGS}} spatial configurations, and {{RESULT_NUM_MODALITIES}} total modality cells. [@fig:taxonomy_matrix] is the central map of that design space. The axes intentionally separate relation from infrastructure: two partners can be relationally engaged while co-present, remote, delayed, or hybrid, but each cell changes what can be perceived, logged, repaired, and revisited.
